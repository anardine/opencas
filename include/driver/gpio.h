...
uint8_t GPIO_ReadFromInputPin(GPIOx_RegTypeDef *pGPIOx, uint8_t pinNumber); // reads the value on the input pin defined
uint32_t GPIO_ReadFromInputPort(GPIOx_RegTypeDef *pGPIOx); // read the entire value of all GPIOx pins
void GPIO_WriteToOutputPin(GPIOx_RegTypeDef *pGPIOx, uint8_t pinNumber, uint8_t dataToWrite); // write the to the output pin
